...
function buildDailyTable(sum, totalS) {
    var h = "<table><thead><tr><th class='sticky-col-item shop-header'>日付</th><th class='shop-header'>予算</th><th class='shop-header'>実績(成約)</th><th class='shop-header'>達成率</th><th class='shop-header'>新規接客</th><th class='shop-header'>再来接客</th><th class='shop-header'>総粗利(込)</th></tr></thead><tbody>";
    Object.keys(sum).sort().forEach(date => {
        var s = sum[date], j = (s.j_k||0)+(s.j_f||0), v = (s.v_n_k||0)+(s.v_n_f||0), rv = (s.rv_k||0)+(s.rv_f||0), ar = (s.ar25_k||0)+(s.ar25_f||0), bud = s.budget_current||0;
        h += "<tr><td class='sticky-col-item' style='background:#f2f2f2; font-weight:bold;'>"+date.split("-")[2]+"日</td><td>"+bud+"</td><td style='background:#ffe599'>"+j+"</td><td>"+(bud>0?Math.round(j/bud*100):0)+"%</td><td>"+v+"</td><td>"+rv+"</td><td style='background:#ead1dc'>"+ar.toLocaleString()+"</td></tr>";
    });
...
function buildTable(sum, title, totalS) {
    ...
    const rowDef = [
        ...
        { lbl: "AB数", m: "ab", cls: "#ffffff" }, { lbl: "即決成約", m: "jk", cls: "#ffffff" }, { lbl: "成約率", type: "ratio", n: "j", d: "v_n", cls: "#ffe599", redText: true },
        { lbl: "再来接客数", m: "rv", cls: "#ffffff" }, { lbl: "総接客数", m: "tot_v", cls: "#ffffff" },
        { sec: "販売区分" }, { lbl: "新車", m: "n", cls: "#b6d7a8" }, { lbl: "未使用車", m: "m", cls: "#b6d7a8" }, { lbl: "中古車", m: "c", cls: "#b6d7a8" },
        { sec: "付帯品個別" }, { lbl: "10年保証", m: "o2", cls: "#a2c4c9" }, { lbl: "パック", m: "pk", cls: "#a2c4c9" }, { lbl: "コーティング", m: "ct", cls: "#a2c4c9" }, { lbl: "下取り", m: "tr", cls: "#a2c4c9" },
...
function renderCell(s, r, isT) {
    ...
    if(r.type === "total_only") { tVal = (s[r.m] || 0).toLocaleString(); return "<td class='"+c+"' style='"+cellStyle+"'><div class='cell-stack' style='align-items:center; font-weight:bold; font-size:12px;'>" + tVal + "</div></td>"; }
    ...
    if(isArari || isDel) {
        ...
        return "<td class='"+c+"' style='"+cellStyle+"'><div class='cell-stack'><div class='bg-sou-upper' style='background:transparent !important; border-bottom:1px dotted #999 !important; font-weight:bold;'>"+tVal+"</div><div class='bg-sou-lower' style='display:flex;'><div class='val-kei'>"+kVal+"</div><div class='val-fu'>"+fVal+"</div></div></div></td>";
    }
    else if(r.lbl === "実績") { kVal = (s.j_k || 0).toLocaleString(); fVal = (s.j_f || 0).toLocaleString(); tVal = ((s.j_k || 0) + (s.j_f || 0)).toLocaleString(); return "<td class='"+c+"' style='"+cellStyle+"'><div class='cell-stack'><div class='stack-label-3'><div>軽</div><div>普</div></div><div class='stack-values-3' style='background:#ffffff !important;'><div class='val-kei'>"+kVal+"</div><div class='val-fu'>"+fVal+"</div></div><div class='stack-total-3' style='background:#ffffff !important;'>"+tVal+"</div></div></td>"; }
