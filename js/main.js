...
function renderAll() {
    ...
    staffStatsMaster = {}; staffStoreMap = {}; storeStatsMaster = {};
    var dailyStats = {}; var totalStaffS = createStats(), totalDailyS = createStats();
    ...
    allData.forEach(r => {
        ...
        storeGroupMap[st] = gr;
        aggregate(storeStatsMaster[st], r);
...
function buildTable(sum, title, totalS) {
    var keys = Object.keys(sum).sort(), h = "<table><thead><tr><th class='sticky-col-item shop-header'>" + (title || "KPI項目") + "</th><th class='sticky-col-total shop-header' style='left:170px;'>合計</th>";
    ...
    const rowDef = [
        ...
        { sec: "納車着地粗利予測" }, { lbl: "納車台数", type: "del_arari_val", val: "del_cnt", cls: "#d9ead3" }, { lbl: "@車両粗利", type: "del_arari_avg", val: "del_ar21", cls: "#d9ead3" }, { lbl: "@ローンBK", type: "del_arari_avg", val: "del_ar23", cls: "#d9ead3" }, { lbl: "@下取粗利", type: "del_arari_avg", val: "del_ar22", cls: "#d9ead3" }, { lbl: "@全部割(保証抜)", type: "del_arari_avg", val: "del_ar24", cls: "#d9ead3" }, { lbl: "@全部割(保証込)", type: "del_arari_avg", val: "del_ar25", cls: "#d9ead3" }, { lbl: "総粗利(保証抜)", type: "del_arari_sum", val: "del_ar24", cls: "#d9ead3" }, { lbl: "総粗利(保証込)", type: "del_arari_sum", val: "del_ar25", cls: "#d9ead3" }, { lbl: "納車台数予算", m: "budget_n", type: "total_only", cls: "#d9ead3" }, { lbl: "達成率", type: "del_total_ratio", n: "del_cnt", d: "budget_n", cls: "#d9ead3" }, { lbl: "総粗利保証込予算", m: "budget_ar", type: "total_only", cls: "#d9ead3" }, { lbl: "達成率", type: "del_total_ratio", n: "del_ar25", d: "budget_ar", cls: "#d9ead3" }
    ];
    for(var j=0; j<rowDef.length; j++){ 
        var r = rowDef[j]; 
        if(r.sec) { h += "<tr><td class='sticky-col-item section-row'>" + r.sec + "</td><td class='sticky-col-total section-row' style='left: 170px;'></td><td colspan='" + keys.length + "' class='section-row'></td></tr>"; } 
...
function filterStoreByGroup() { 
    var g = document.getElementById('group-selector').value, f = {}, t = createStats(); 
    Object.keys(storeStatsMaster).forEach(function(st){ if (g === "all" || storeGroupMap[st] === g) { f[st] = storeStatsMaster[st]; Object.keys(t).forEach(function(k){ if(typeof t[k] === 'number') t[k] += storeStatsMaster[st][k]; }); } }); 
    document.getElementById("store-table-container").innerHTML = buildTable(f, "店舗名", t); 
}
